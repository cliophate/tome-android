...
class MediaLibraryTreeTest {
  ...
  private lateinit var preferences: TomeSharedPreferences
  private lateinit var localCacheRepository: LocalCacheRepository
  private lateinit var mediaProvider: TomeMediaProvider
  ...
  fun setUp() {
    ...
    mediaProvider = mockk(relaxed = true)
    ...
    coEvery { mediaProvider.fetchRecentListenedBooks(any()) } returns
      OperationResult.Error(OperationError.InternalError)
    coEvery { mediaProvider.fetchBooks(any(), any(), any()) } returns
      OperationResult.Error(OperationError.InternalError)
    coEvery { localCacheRepository.fetchDetailedItems() } returns
      OperationResult.Error(OperationError.InternalError)

    tree = MediaLibraryTree(context, preferences, localCacheRepository, mediaProvider)
  }
  ...
  fun getChildren_recent_returnsEmptyWhenProviderFails() =
    runBlocking {
      ...
      coEvery { mediaProvider.fetchRecentListenedBooks("lib-1") } returns
  ...
  fun getChildren_nestedLibrary_returnsBooksFromLibrary() =
    runBlocking {
      ...
      coEvery { mediaProvider.fetchBooks("lib-1", any(), any()) } returns
        OperationResult.Success(
          PagedItems(
            items =
              listOf(
  ...
  fun getItem_bookPath_returnsBookItem() =
    runBlocking {
      coEvery { mediaProvider.fetchBook("book-1") } returns
  ...
  fun getItem_bookPath_returnsErrorWhenBookNotFound() =
    runBlocking {
      coEvery { mediaProvider.fetchBook("missing-id") } returns
  ...
  fun searchBooks_returnsEmptyWhenProviderFails() =
    runBlocking {
      every { preferences.getPreferredLibrary() } returns makeLibrary("lib-1")
      coEvery { mediaProvider.searchBooks("lib-1", "audiobook", any()) } returns
        OperationResult.Error(OperationError.NetworkError)
      val results = tree.searchBooks("audiobook").get()
      assertTrue(results.isEmpty())
    }
  ...
  fun searchBooks_returnsMatchingBooks() =
    runBlocking {
      every { preferences.getPreferredLibrary() } returns makeLibrary("lib-1")
      coEvery { mediaProvider.searchBooks("lib-1", "dune", any()) } returns
        OperationResult.Success(
          listOf(
